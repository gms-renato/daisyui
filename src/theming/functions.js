...
const colorIsInvalid = (input) => {
    console.error(
        `├─ ${pc.red("⚠︎")} ${pc.bgRed(" Error ")} Invalid color ${pc.red(
            input
        )} in ${pc.green("tailwind.config.js")}`
    );
};
...
module.exports = {
    ...
    colorObjToString: (input) => {
        ...
        return `${Number.parseFloat(
            (cutNumber(l) * 100).toFixed(6)
        )}% ${cutNumber(c)} ${cutNumber(h)}`;
    },
    ...
    convertColorFormat: function (input) {
        ...
        for (const [rule, value] of Object.entries(input)) {
            ...
            if (!Object.hasOwn(input, "accent-content")) {
                resultObj["--ac"] = this.generateForegroundColorFrom(
                    input.accent,
                    0.8
                );
            }
            ...
            for (const item of config("daisyui.themes")) {
                if (typeof item === "object" && item !== null) {
                    for (const [
                        ...
                        includedThemesObj[customThemeName] =
